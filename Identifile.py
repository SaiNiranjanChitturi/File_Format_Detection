...
class Identifile:
    ...
    def is_compressed(self) -> bool:
        ...
        return self.format.lower() in {
            ...
            "xz", "snappy-framed", "snappy-snz", "snappy-raw", "brotli"
        }
...
SIGNATURES = {
    ...
    "snappy-snz": {
        "start": [b"SNZ\x01"],
        "evidence": "Starts with 'SNZ\\x01' (obsolete Snappy SNZ format).",
    },
    "brotli": {
...
def sniff_format(file_path: str, head_n: int = 64, tail_n: int = 64,
    ...
    try:
        with open(file_path, "rb") as fp:
            ...
            detection = _detect_from_ranges(head, tail, tar_slice)
    ...
    try:
        if hasattr(stream, "seekable") and stream.seekable():
            ...
            if not buffer_non_seekable:
                ...
                detection = _detect_from_ranges(head, b"", b"")
                if detection.is_known():
                    detection.confidence *= 0.8  # Reduce confidence for partial
                    detection.evidence += " (partial detection from head only)."
            else:
